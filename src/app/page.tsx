...
function HowToGetCarSection() {
  ...
  return (
    <section className="w-full py-24 px-6 relative overflow-hidden bg-card">
      ...
      <div className="w-full max-w-screen-2xl mx-auto relative z-10">
        <div className="mb-16 md:mb-20 text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-8 border-b border-slate-200/60 pb-8">
          <div>
            ...
            <h2 className="text-3xl lg:text-5xl font-bold text-foreground leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-amber-500">ИЗ ДРУГИХ СТРАН, ОТ А ДО Я</span>
            </h2>
          </div>
          <div className="w-full md:w-[600px] h-[500px] bg-background border border-slate-200/60 rounded-2xl overflow-hidden shadow-2xl h-full relative group">
              
              {/* Decorative glows */}
              <div className="absolute -top-20 -left-20 w-40 h-40 bg-primary/30 rounded-full blur-[50px] z-20 pointer-events-none opacity-50" />
              <div className="absolute -bottom-20 -right-20 w-40 h-40 bg-amber-500/30 rounded-full blur-[50px] z-20 pointer-events-none opacity-50" />
                ...
                {reviews.map((review, index) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-background via-background/80 to-transparent flex flex-col justify-end p-8">
                      <div className="glass-panel border border-slate-200/60 rounded-2xl p-6 backdrop-blur-md transform translate-y-0 opacity-100 transition-all duration-500 delay-200 shadow-2xl">
                        
                        <div className="flex items-center justify-between gap-2 mb-4">
                          <h4 className="font-bold text-xl text-foreground drop-shadow-md">{review.name}</h4>
                        ...
                        <div className="flex items-center gap-4 pt-4 border-t border-slate-200/60">
                          <div className="flex bg-slate-50 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-slate-200/60 items-center gap-2">
                             <div className="text-xs text-muted-foreground font-medium">Яндекс</div>
                             <div className="text-sm text-foreground font-bold">4.9</div>
                          </div>
              ...
              <div className="absolute top-1/2 -translate-y-1/2 inset-x-4 flex justify-between z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button
                  ...
                  className="w-12 h-12 bg-background/50 hover:bg-primary/80 backdrop-blur-md rounded-full flex items-center justify-center text-foreground border border-slate-200/60 transition-all hover:scale-110 shadow-lg"
                >
              ...
              {/* Progress Dots */}
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-30 bg-background/50 backdrop-blur-md px-3 py-2 rounded-full border border-slate-200/60">
                {reviews.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setCurrentReview(index)}
                    className={`h-2 rounded-full transition-all duration-300 ${
                      index === currentReview ? 'bg-primary w-6 shadow-md' : 'bg-slate-50 w-2 hover:bg-slate-50'
                    }`}
                  />
                ))}
              </div>
